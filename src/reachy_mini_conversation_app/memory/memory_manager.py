...
def _slugify(text: str, *, max_len: int = 40) -> str:
    """Turn arbitrary text into an id-safe slug (a–z, 0–9, hyphen).

    Must produce something matching the slug portion of ``_MEMORY_ID_PATTERN``:
    starts with an alphanumeric, then lowercase alphanumerics / hyphens / underscores.
    Falls back to ``memory`` when the input has no usable characters.
    """
    cleaned = re.sub(r"[^a-z0-9]+", "-", text.lower()).strip("-")
    cleaned = cleaned[:max_len].strip("-")
    if not cleaned or not cleaned[0].isalnum():
        cleaned = ("m" + cleaned).strip("-") if cleaned else "memory"
    return cleaned
...
class MemoryManager:
    ...
    # ------------------------------------------------------------------
    # Synchronous live writes (DM-facing remember/forget)
    # ------------------------------------------------------------------

    def _new_memory_id(self, text: str, *, created: datetime | None = None) -> str:
        """Build a fresh, collision-free memory id from a body/title hint.

        Format ``YYYY-MM-DD_<slug>_<3-hex>`` (matches ``_MEMORY_ID_PATTERN``). The
        3-hex suffix is random; on the rare collision we re-roll it.
        """
        day = (created or _now_utc()).strftime("%Y-%m-%d")
        slug = _slugify(_one_line_summary(text) or text)
        for _ in range(8):
            memory_id = f"{day}_{slug}_{secrets.token_hex(2)[:3]}"
            if not self.memory_exists(memory_id):
                return memory_id
        # Extremely unlikely; fall back to a time-based suffix slug.
        return f"{day}_{slug}-{secrets.token_hex(2)}_{secrets.token_hex(2)[:3]}"
    ...
    def save_memory_sync(
        ...
        *,
        ...
        """Immediately persist ONE memory and rebuild the index. Returns the new id.

        Thin wrapper over :py:meth:`write_memory` (same atomic temp-file + os.replace
        path and frontmatter/id format the dreamer uses), so live "remember" writes
        coexist with the offline dreamer. Validates ``kind`` against the closed vocab.
        """
        if kind not in ALLOWED_KINDS:
            raise ValueError(f"invalid kind: {kind!r}. One of {sorted(ALLOWED_KINDS)}")
        body = (text or "").strip()
        ...
        memory_id = self._new_memory_id(body)
        self.write_memory(
            memory_id,
            body,
            kind=kind,
        ...
        logger.info("save_memory_sync wrote %s (kind=%s)", memory_id, kind)
        return memory_id

    def forget_memory(self, id_or_query: str) -> dict[str, Any]:
        """Archive a memory by marking it superseded (never hard-deletes).

        Accepts an exact memory id or a free-text query (best substring match via
        :py:meth:`find_related_memories`). Sets ``superseded_by="forgotten"`` so the
        memory drops out of the index/recall but the file is preserved (mirrors the
        dreamer's supersede-don't-delete invariant). Returns a small status dict.
        """
        query = (id_or_query or "").strip()
        if not query:
            return {"status": "error", "message": "id or query is required"}

        target_id: str | None = None
        if self.memory_exists(query):
            target_id = query
        else:
            matches = self.find_related_memories(query=query, limit=1)
            if matches:
        ...
        try:
            self.update_memory(target_id, frontmatter_updates={"superseded_by": "forgotten"})
        except (FileNotFoundError, ValueError) as e:
            return {"status": "error", "id": target_id, "message": str(e)}
        self._rebuild_index_safe()
        logger.info("forget_memory archived %s", target_id)
        return {"status": "forgotten", "id": target_id}

    def _rebuild_index_safe(self) -> None:
        """Rebuild active_memory.md from frontmatter; best-effort (no LLM)."""
        try:
            from reachy_mini_conversation_app.memory.index_renderer import rebuild_index

            rebuild_index(self)
        except Exception as e:
            logger.warning("Failed to rebuild active_memory.md after write: %s", e)
