...
export async function mountSettingsView({ outlet, signal }) {
  ...
  const backendSection = buildBackendSection({
    onSaved: () =>
      Promise.all([
        refreshStatus({ statusSection, backendSection, mcpSection, signal }),
  ...
  const mcpSection = buildMcpSection({
    onSaved: () => refreshStatus({ statusSection, backendSection, mcpSection, signal }),
  });
...
async function refreshStatus({ statusSection, backendSection, mcpSection, signal }) {
  try {
