...
Installed public Hugging Face Space tools can also be enabled here after you add them with `tool-spaces`, and generic remote MCP server tools after you add them with `mcp-servers` (see below).
...
<details>
<summary><b>Generic remote MCP servers</b></summary>

Beyond public Hugging Face Spaces, you can connect the app to **any** HTTP(S) MCP server
(Streamable HTTP transport). Its tools then become callable by Reachy, namespaced under a local alias.

```bash
# register a server (alias "example") and enable its tools in the active profile
reachy-mini-conversation-app mcp-servers add example \
  http://my-mcp-server.local:8000/mcp \
  --token-env MCP_SERVER_TOKEN

# register without enabling in any profile
reachy-mini-conversation-app mcp-servers add example <url> --token-env MCP_SERVER_TOKEN --install-only

# list configured servers and their discovered tools
reachy-mini-conversation-app mcp-servers list

# remove a configured server
reachy-mini-conversation-app mcp-servers remove example
```

Notes:

- **Tools are namespaced** as `<alias>__<ToolName>` (e.g. `example__DoThing`) and enabled by
